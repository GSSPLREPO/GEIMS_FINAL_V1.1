...
{
    public partial class SchoolPurchase : PageBase
    {
        ...
        protected void btnViewSchools_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hfValidate.Value == "1")
                {
                    ...
                    TrustBL objTrust = new TrustBL();
                    if (rblSchoolOrTrust.SelectedValue == "0")
                    {
                        objResult =
                            objMaterialGroupBL.School_SelectAll_ForDropDOwn(
                                Convert.ToInt32(Session[ApplicationSession.TRUSTID]));
                        DataRow[] drr = objResult.resultDT.Select("SchoolMID=' " + Convert.ToInt32(Session[ApplicationSession.SCHOOLID]) + " ' ");
                        for (int i = 0; i < drr.Length; i++)
                            drr[i].Delete();
                        objResult.resultDT.AcceptChanges();
                        if (objResult != null)
                        {
                            ...
                            if (objResult.resultDT.Rows.Count > 0)
                            {
                                ...
                                gvMaterialForTrust.Visible = false;
                            }
                            else
                            {
                                gvSchoolForMaterial.Visible = false;
                    ...
                    {
                        objResult =
                            objTrust.Trust_Select(Convert.ToInt32(Session[ApplicationSession.TRUSTID]));
                        if (objResult != null)
                        {
                            gvMaterialForTrust.DataSource = objResult.resultDT;
                            gvMaterialForTrust.DataBind();

                            if (objResult.resultDT.Rows.Count > 0)
                            {
                                gvMaterialForTrust.Visible = true;
                                gvSchoolForMaterial.Visible = false;
                            }
                            else
                            {
                ...
                if (hfValidate.Value == "1")
                {
                    ...
                    if (objResults.resultDT.Rows[0][0].ToString() == "0")
                    {
                        ...
                        if (rblMaterialSelection.SelectedValue == "0")
                        {
                            ...
                            {
                                foreach (GridViewRow row in gvMaterialForTrust.Rows)
                                {
                        ...
                        {
                            foreach (GridViewRow row in gvMultipleMaterial0.Rows)
                            {
                                DropDownList ddlgridSchool;
                                if (rblSchoolOrTrust.SelectedValue == "0")
                                {
                                    ddlgridSchool = (DropDownList)row.Cells[8].FindControl("ddlGridSchool0");
                                }
                                else
                                {
                                    ddlgridSchool = (DropDownList)row.Cells[7].FindControl("ddlGridTrust");
                                }

                                objMaterialTransferBO.MaterialTID = Convert.ToInt32(row.Cells[0].Text);
                                objMaterialTransferBO.Quantity =
                                    Convert.ToInt32(((TextBox)row.FindControl("txtQuantity0")).Text);
                        ...
                        if (objResults.status == ApplicationResult.CommonStatusType.SUCCESS)
                        {
                            ...
                            {
                                ScriptManager.RegisterStartupScript(this, GetType(), "alert", "alert('" + strMessage + "');",
                                    true);
                            }
                            else
                            {
                                ...
                                ScriptManager.RegisterStartupScript(this, GetType(), "CallMyFunction", "ClearOnTransfer();", true);
                                ViewState["MaterialNameID"] = null; ViewState["UomTID"] = null; ViewState["MaterialIDs"] = null;
                                hfTab.Value = "1";
                                gvMaterialForTrust.Visible = false;
                                gvSchoolForMaterial.Visible = false;
        ...
        protected void btnAddMaterial_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (hfValidate.Value == "1")
                {
                    ...
                    if (objResult != null)
                    {
                        gvMultipleMaterial0.DataSource = objResult.resultDT;
                        gvMultipleMaterial0.DataBind();

                        if (objResult.resultDT.Rows.Count > 0)
                        {
                            gvMultipleMaterial0.Visible = true;
                        }
                        else
                        {
                            gvMultipleMaterial0.Visible = false;
        ...
        protected void gvMultipleMaterial0_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                SchoolBL objSchoolBl = new SchoolBL();
                ...
                ApplicationResult objResults = new ApplicationResult();
                Controls objControls = new Controls();
                if (rblSchoolOrTrust.SelectedValue == "1")
                {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    DropDownList ddlSchool = (DropDownList)e.Row.FindControl("ddlGridSchool0");

                    objResults = objSchoolBl.School_SelectAll_ForDropDOwn(Convert.ToInt32(Session[ApplicationSession.TRUSTID]));
                    DataRow[] drr = objResults.resultDT.Select("SchoolMID=' " + Convert.ToInt32(Session[ApplicationSession.SCHOOLID]) + " ' ");
                    for (int i = 0; i < drr.Length; i++)
                        drr[i].Delete();
                    objResults.resultDT.AcceptChanges();
                    if (objResults != null)
                    {
                        if (objResults.resultDT.Rows.Count > 0)
                        {
                    ...
                    DropDownList ddlTrust = (DropDownList)e.Row.FindControl("ddlGridTrust");

                    objResults = objTrust.Trust_Select(Convert.ToInt32(Session[ApplicationSession.TRUSTID]));
                    if (objResults != null)
                    {
                        if (objResults.resultDT.Rows.Count > 0)
                        {
                            objControls.BindDropDown_ListBox(objResults.resultDT, ddlTrust, "TrustNameEng", "TrustMID");
                        }
